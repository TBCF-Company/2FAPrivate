...
export class OtpManager {
  ...
  constructor(digits: number = 6, period: number = 30) {
    this.digits = digits;
    this.period = period;
  ...
  async generateTotp(secret: string): Promise<string> {
    try {
      const code = await this.totp.generate({ secret, digits: this.digits as any, period: this.period });
      return code;
    } catch (error) {
   ...
   * Validate TOTP code
   */
  async validateTotp(secret: string, code: string, window: number = 1): Promise<boolean> {
    try {
      const result = await this.totp.verify(code, { secret, digits: this.digits as any, period: this.period });
      return result.valid;
    } catch (error) {
